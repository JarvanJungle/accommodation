//
// This file was generated by the JavaTM Architecture for XML Binding(JAXB) Reference Implementation, v2.3.2 
// See <a href="https://javaee.github.io/jaxb-v2/">https://javaee.github.io/jaxb-v2/</a> 
// Any modifications to this file will be lost upon recompilation of the source schema. 
// Generated on: 2021.06.10 at 10:00:19 PM ICT 
//


package com.sabre.schema.hotel.pricecheck.v4_0_0;

import javax.xml.bind.annotation.XmlEnum;
import javax.xml.bind.annotation.XmlEnumValue;
import javax.xml.bind.annotation.XmlType;


/**
 * <p>Java class for OffsetDropTimeType.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * <p>
 * <pre>
 * &lt;simpleType name="OffsetDropTimeType"&gt;
 *   &lt;restriction base="{http://www.w3.org/2001/XMLSchema}string"&gt;
 *     &lt;enumeration value="BeforeArrival"/&gt;
 *     &lt;enumeration value="AfterBooking"/&gt;
 *     &lt;enumeration value="AfterConfirmation"/&gt;
 *     &lt;enumeration value="AfterArrival"/&gt;
 *     &lt;enumeration value="AfterDeparture"/&gt;
 *   &lt;/restriction&gt;
 * &lt;/simpleType&gt;
 * </pre>
 * 
 */
@XmlType(namespace = "checkRate", name = "OffsetDropTimeType")
@XmlEnum
public enum OffsetDropTimeType {

    @XmlEnumValue("BeforeArrival")
    BEFORE_ARRIVAL("BeforeArrival"),
    @XmlEnumValue("AfterBooking")
    AFTER_BOOKING("AfterBooking"),
    @XmlEnumValue("AfterConfirmation")
    AFTER_CONFIRMATION("AfterConfirmation"),
    @XmlEnumValue("AfterArrival")
    AFTER_ARRIVAL("AfterArrival"),
    @XmlEnumValue("AfterDeparture")
    AFTER_DEPARTURE("AfterDeparture");
    private final String value;

    OffsetDropTimeType(String v) {
        value = v;
    }

    public String value() {
        return value;
    }

    public static OffsetDropTimeType fromValue(String v) {
        for (OffsetDropTimeType c: OffsetDropTimeType.values()) {
            if (c.value.equals(v)) {
                return c;
            }
        }
        throw new IllegalArgumentException(v);
    }

}
