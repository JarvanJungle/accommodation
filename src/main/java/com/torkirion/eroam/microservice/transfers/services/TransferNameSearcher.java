...
@Slf4j
public class TransferNameSearcher
{
	@Autowired
	...
	public void prime()
	{
		...
		if (applicationConfig.getProduct().isTransfers())
		{
			long startTime = System.currentTimeMillis();
			try
			{
				File baseDir = new File(System.getProperty("java.io.tmpdir") + "/transferNameSearcher");
				...
				int loadPageSize = applicationConfig.getTransferNameSearcher().getLoadPageSize();
				...
				do
				{
					...
					for (AccommodationRCRepo.CodeAndName codeAndName : codeAndNames)
					{
						...
						if (hotelAllowedInNameSearch(codeAndName.getCode()))
						{
							...
							document.add(new StringField("type", EndpointType.HOTEL.toString(), Store.YES));
				...
				long airportTime = System.currentTimeMillis();
				counter = 0;
				List<AirportTerminalData> airportTerminals = airportTerminalRepo.findAll();
				for (AirportTerminalData airportTerminal : airportTerminals)
				{
					counter++;
					Document document = new Document();
					document.add(new TextField("name", airportTerminal.getFullAirportTerminalName(), Store.YES));
					document.add(new StringField("code", airportTerminal.getIata() + " " + airportTerminal.getTerminalName(), Store.YES));
					document.add(new StringField("type", EndpointType.AIRPORT.toString(), Store.YES));
					// document.add(new StringField("country", country, Store.YES));
					writer.addDocument(document);
				}
				writer.close();
				log.info("prime::loaded " + counter + " airport names index in " + (System.currentTimeMillis() - airportTime) + " millis");
	...
	protected List<NameMatch> getBooleanTermMatches(String term, Set<String> dupCheck)
	{
		...
		try
		{
			...
			for (int i = 0; i < hits.length; ++i)
			{
				...
				{
					results.add(new NameMatch(type, code, name));
					dupCheck.add(name);
					if (results.size() >= MAX_RESULTS)
						break;
				}
