...
public class HotelbedsInterface
{
	...
	public HotelbedsAPIProperties getHotelbedsPropertiesx()
	{
		return hotelbedsProperties;
	}

	public HotelbedsInterface(SystemPropertiesDAO properties, String site, String channel) throws Exception
	{
	...
	private void init(String site, String channel) throws Exception
	{
		...
		hotelbedsProperties = new HotelbedsAPIProperties(properties, site);
	...
	public AvailabilityRS startSearchHotels(AvailSearchByHotelIdRQDTO availSearchRQ) throws Exception
	{
		...
		for (String hotelId : availSearchRQ.getHotelIds())
		{
			...
			if (hotelId.startsWith(HotelbedsService.CHANNEL_PREFIX))
				hotelId = hotelId.substring(HotelbedsService.CHANNEL_PREFIX.length());
	...
	private AvailabilityRQ makeAvailabilityRQ(AvailSearchRQDTO availSearchRQ)
	{
		...
		availabilityRQ.setSourceMarket(hotelbedsProperties.sourceMarket);
		int roomNumber = 1;
	...
	public CheckRateRS checkRates(List<String> ratekeys) throws Exception
	{
		if (log.isDebugEnabled())
	...
	protected AccommodationBookRS bookSingle(AccommodationBookRQ bookRQ) throws Exception
	{
		...
		try
		{
			...
			if (hotelbedsProperties.bypassBooking)
			{
	...
	{
		return hotelbedsProperties.allowZeroCommissionProduct;
	}
	/*
