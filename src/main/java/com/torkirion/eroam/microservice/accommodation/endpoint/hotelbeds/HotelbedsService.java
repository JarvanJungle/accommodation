...
@AllArgsConstructor
@Slf4j
public class HotelbedsService implements AccommodationServiceIF
{
	...
	public static final String CHANNEL = "HOTELBEDS";
		...
		try
		{
			...
			else
			{
				...
				for (HotelResponse hotelResponse : availabilityRS.getHotels().getHotel())
				{
					...
					accommodationResult.setRooms(loadRooms(hotelBedsInterface, hotelResponse.getRooms().getRoom(), hotelResponse.getCurrency(), availSearchRQ, property_id));
	...
	public AccommodationRateCheckRS rateCheck(RateCheckRQDTO rateCheckRQDTO)
	{
		...
		try
		{
			HotelbedsInterface hotelBedsInterface = new HotelbedsInterface(propertiesDAO, rateCheckRQDTO.getClient(), CHANNEL);
			...
			for (String rateKeyQuery : rateCheckRQDTO.getBookingCodes())
			{
				...
				CheckRateRS checkRateRS = hotelBedsInterface.checkRates(rateKeys);
						...
						rateLoop: for (RateHotelResponse rateHotelResponse : roomHotelResponse.getRates().getRate())
						{
							for (String rateKey : rateKeys)
							{
								...
								if (rateHotelResponse.getRateKey().equals(rateKey))
								{
									...
									{
										loadRateComments(hotelBedsInterface, rateHotelResponse.getRateCommentsId(), property.getCode(), rateCheckRQDTO.getCheckin(), room);
									}
	...
	public AccommodationBookRS book(String site, AccommodationBookRQ bookRQ)
	{
		...
		try
		{
			HotelbedsInterface hotelBedsInterface = new HotelbedsInterface(propertiesDAO, site, CHANNEL);
			AccommodationBookRS bookRS = hotelBedsInterface.book(bookRQ);
	...
	public AccommodationCancelRS cancel(String site, AccommodationCancelRQ cancelRQ)
	{
		...
		{
			HotelbedsInterface hotelBedsInterface = new HotelbedsInterface(propertiesDAO, site, CHANNEL);
			AccommodationCancelRS cancelRS = hotelBedsInterface.cancel(cancelRQ);
	...
	public AccommodationRetrieveRS retrieve(String site, AccommodationRetrieveRQ retrieveRQ)
	{
		...
		{
			HotelbedsInterface hotelBedsInterface = new HotelbedsInterface(propertiesDAO, site, CHANNEL);
			AccommodationRetrieveRS retrieveRS = hotelBedsInterface.retrieve(retrieveRQ);
	...
	private SortedSet<RoomResult> loadRooms(HotelbedsInterface hotelBedsInterface, List<RoomHotelResponse> roomRateDetailsList, String currency, AvailSearchRQDTO availSearchRQ,
		...
		for (RoomHotelResponse roomHotelResponse : roomRateDetailsList)
		{
			...
			rateLoop: for (RateHotelResponse rateHotelResponse : roomHotelResponse.getRates().getRate())
			{
				...
				for ( int roomIdx = 1; roomIdx <= roomCount; roomIdx++)
				{
					...
					if (rateHotelResponse.getRateCommentsId() != null && rateHotelResponse.getRateCommentsId().length() > 0)
					{
						loadRateComments(hotelBedsInterface, rateHotelResponse.getRateCommentsId(), hotelbedsHotelCode, availSearchRQ.getCheckin(), room);
					}
					makeRoomPromotions(room, rateHotelResponse);
			...
			String currency) throws Exception
	{
		...
		{
			totalPrice_bd = HotelbedsInterface.applyInventoryMarkup(rateHotelResponse.getNet(), null);
			if (log.isDebugEnabled())
	...
	private void loadRateComments(HotelbedsInterface hotelBedsInterface, String rateCommentsID, String hotelID, LocalDate checkin, RoomResult room)
	{
		if (log.isDebugEnabled())
		...
		try
		{
			Set<HotelbedsCache.RateComment> comments = getHotelbedsCache().getRateComments(fullCode);

			for (HotelbedsCache.RateComment comment : comments)
			{
				if (log.isDebugEnabled())
	...
	private CNXDetails makeCNXPolicy(RateHotelResponse rateHotelResponse, String currency, LocalDate checkin, LocalDate checkout, BigDecimal totalPrice_bd, boolean markupCNXValues) throws Exception
	{
		...
		if (rateHotelResponse.getCancellationPolicies() != null && rateHotelResponse.getCancellationPolicies().getCancellationPolicy().size() > 0)
		{
			for (CancellationPolicy cancellationPolicy : rateHotelResponse.getCancellationPolicies().getCancellationPolicy())
			{
				...
				if (markupCNXValues)
					penaltyAmount = HotelbedsInterface.applyInventoryMarkup(cancellationPolicy.getAmount(), null);
	...
	private HotelbedsCache getHotelbedsCache()
	{
		if (_hotelbedsCache == null)
			_hotelbedsCache = new HotelbedsCache(staticRepo);
		return _hotelbedsCache;
	}
